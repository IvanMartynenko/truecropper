...
| TrueCropperOptionsError | The specified aspect ratio (${aspectRatio}) and calculated ${minimum/StartSize/DefaultSize} dimensions (width/height = ${calculatedAspectRatio}) are greater than (${epsilon}). This might be due to a rounding error on the server side or incorrect minimum sizes. | null | Checking for possible floating point rounding errors in aspect ratio |
